...
export function createScene({
  ...
  function loadMode(modeCfg) {
    ...
    for (const prop of modeCfg.props || []) {
      propsEl.append(makeImg(prop.sprite, 'tw-prop', prop));
    }
  ...
  function settleInScene(critter, xPct, yPct, homed) {
    critter.el.classList.remove('is-carried');
    critter.el.style.transform = '';
  ...
  function land(handle, target, from) {
    ...
    // landing slot in scene percent (spread capacity-2 targets)
    const slotIndex = target.filled.length - 1;
    const spread = (target.cfg.capacity || 1) > 1 ? (slotIndex === 0 ? -0.24 : 0.24) : 0;
    const dxPct = spread * ((target.cfg.w || target.cfg.pad || 200) / sceneW) * 100;
    const xPct = target.cfg.x + dxPct;
    const yPct = target.cfg.y + (target.cfg.sprite ? 1.5 : 0);
    ...
    return new Promise((resolve) => {
      ...
      if (reducedMotion()) {
        ...
        setTimeout(() => {
          settleInScene(critter, xPct, yPct, true);
      ...
      runAnim((now) => {
        ...
        // settle bounce in the last fifth
        const squash = p > 0.86 ? 1 - 0.14 * Math.sin(((p - 0.86) / 0.14) * Math.PI) : 1;
        critter.el.style.transform = `translate(${x}px, ${y}px) translate(-50%, -50%) scale(${2 - squash}, ${squash})`;
        return p < 1;
      }, () => {
        settleInScene(critter, xPct, yPct, true);
        critter.status = 'homed';
