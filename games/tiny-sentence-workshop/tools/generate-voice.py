#!/usr/bin/env python3
"""Generate and Whisper-QA every Tiny Sentence Workshop teacher-voice line.
...
def generation_lock():
    ...
    lock_path = Path(tempfile.gettempdir()) / f"qlobe-tiny-sentence-workshop-voice-{identity}.lock"
...
def generate_batch(args: argparse.Namespace, lines: dict[str, str], api: str, ref: str) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="tiny-sentence-workshop-voice-") as folder:
        stage = Path(folder)
        for seed in SEEDS:
            if not pending:
                break
...
def main() -> int:
    ...
    try:
        ...
        print("another Tiny Sentence Workshop voice batch is already running", file=sys.stderr)
        return 2
