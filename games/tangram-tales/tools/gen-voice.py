#!/usr/bin/env python3
"""Produce Tangram Tales teacher-voice clips with transcript QA.
...
def transcript_score(wanted: str, heard: str) -> tuple[bool, float, float]:
    ...
    accepted = got == want if len(want_words) <= 3 else same_letters or ratio >= .86 and coverage >= .92
...
def main() -> int:
    ...
    reference_value = args.voice_ref or os.environ.get("QLOBE_VOICE_REF") or state.get("teacherVoicePath") or ""
        ...
        for seed in args.seeds:
            ...
            ok = upload(
                ...
                [f"audio=@{candidate}", "model_size=base", "language=en",
                 "initial_prompt=Tangram Tales. Coral, blue, orange, violet, teal, yellow, green. "
                 "Triangle, square, parallelogram. Fox, fish, bird."],
                transcript_file,
            )
