...
| Narrator voice clips ×112 | `assets/audio/*.m4a` + `manifest.json` + `lines.json` | done | teacher-voice clone (shared canonical ref, seed 7), transcription-QA'd, 2026-07-20 (incl. judge-flow lines) |
| Judge buttons (plush happy/sad faces) | `assets/ui/judge-sad.png`, `assets/ui/judge-happy.png` | done | user-supplied plush-face art, circular-cropped from supplied render, 2026-07-20 |
| Hub tile | `../../assets/hub/tiles/puppet-problem-solvers.jpg` | done | local GenAI (krea2, seed 42), 640×533 JPEG, 2026-07-20 |

All assets are live. Documented fallbacks remain in the engine (tinted props,
emoji judge faces, Web Speech) so a missing file degrades gracefully rather
than blocking play.
