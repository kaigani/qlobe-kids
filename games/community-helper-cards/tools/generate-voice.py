#!/usr/bin/env python3
"""Generate and Whisper-QA Community Helper Cards teacher-voice lines.
...
def generate_batch(args: argparse.Namespace, lines: dict[str, str], api: str, ref: str) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="community-helper-cards-voice-") as folder:
        stage = Path(folder)
        for seed in SEEDS:
            if not pending:
                break
...
def main() -> int:
    ...
    try:
        ...
        print("another Community Helper Cards voice batch is already running", file=sys.stderr)
        return 2
