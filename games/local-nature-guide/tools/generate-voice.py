#!/usr/bin/env python3
"""Generate and Whisper-QA Local Nature Guide teacher-voice lines.
...
QA_PATH = GAME / "assets/source/voice/qa-transcripts.json"
...
def norm(value: str) -> str:
    ...
    value = re.sub(r"\b(let's)\b", "let us", value)
...
def score(key: str, expected: str, heard: str) -> tuple[bool, float]:
    ...
    word = key[5:] if key.startswith("word-") else ""
    content_ok = not word or norm(word) in transcript
    return (ratio >= 0.90 or intended == transcript) and content_ok, round(ratio, 3)


def minimum_duration(key: str) -> float:
    return 0.2 if key.startswith("word-") else 0.35
...
def transcribe_candidate(
    ...
    audio_ok = (
        ...
        and minimum_duration(key) <= clip_duration <= 20
...
def generate_batch(args: argparse.Namespace, lines: dict[str, str], api: str, ref: str) -> int:
    ...
    for key, text in lines.items():
        ...
        reusable = (
            ...
            and duration(path) >= minimum_duration(key)
        )
        if reusable:
            qa[key] = {
                **prior,
    ...
    with tempfile.TemporaryDirectory(prefix="local-nature-guide-voice-") as folder:
        stage = Path(folder)
        for seed in SEEDS:
            if not pending:
                break
    ...
    manifest = {"_v": "local-nature-guide-voice-1"}
    manifest.update({
        key: {
            "file": f"{key}.m4a",
...
def check_batch(lines: dict[str, str]) -> int:
    try:
        ...
        return 1
    failures = []
    manifest_keys = set(manifest) - {"_v"} if isinstance(manifest, dict) else set()
    if manifest_keys != set(lines):
    ...
    for key, text in lines.items():
        ...
        valid = (
            ...
            and minimum_duration(key) <= duration(path) <= 20
        )
        if not valid:
            failures.append(key)
    ...
    try:
        with generation_lock():
            ...
            try:
                ...
                ref = configured_ref if configured_ref and Path(configured_ref).is_file() else str(PLATFORM_VOICE)
        ...
        print("another Local Nature Guide voice batch is already running", file=sys.stderr)
        return 2
