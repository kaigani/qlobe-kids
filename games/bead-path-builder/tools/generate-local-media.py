#!/usr/bin/env python3
"""Resumable LAN media generation for Bead Path Builder.
...
def transcribe_one(
    *,
    ...
    if not heard:
        raise SystemExit("Whisper result did not contain transcript text")
...
def video_command(args: argparse.Namespace, cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in read_manifest(args.manifest):
        ...
        recipe = {
            ...
            "seed": fields["seed"],
            "dimensions": {"width": fields["width"], "height": fields["height"]},
            "duration": fields["duration"],
            "references": {key: relative(value) for key, value in file_paths.items()},
            "output": relative(output),
            "result": video_result,
        }
        atomic_json(qa_dir / f"{key}.video.recipe.json", recipe)
        results.append(recipe)
    return results
...
def transcribe_command(args: argparse.Namespace, cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    output = safe_path(
        args.output or f"games/bead-path-builder/assets/source/qa/{source.stem}.transcript.txt",
        "transcript output",
    )
...
def parser() -> argparse.ArgumentParser:
    ...
    voice.add_argument("--output-dir", default="games/bead-path-builder/assets/source/voice-raw")
    voice.add_argument("--qa-dir", default="games/bead-path-builder/assets/source/qa")
    ...
    video.add_argument(
        "--output-dir", default="games/bead-path-builder/assets/source/video-raw"
    )
    video.add_argument("--qa-dir", default="games/bead-path-builder/assets/source/qa")
    ...
    layered.add_argument("--qa-dir", default="games/bead-path-builder/assets/source/qa")
...
def main() -> None:
    ...
    cfg = load_state(require_endpoint=args.execute)
