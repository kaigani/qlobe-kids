...
function complete({ resume = false } = {}) {
  ...
  const heading = state.mode === 'free' ? 'Your necklace shines!' : `${state.mode.toUpperCase()} pattern complete!`;
  setMarkup(`<section class="screen"><div class="board-shell complete-board"><h1>${heading}</h1><div class="necklace">${state.placed.map((id, i) => `<img src="${asset(id)}" alt="${config.beads[id].name} bead" style="left:${POINTS[i][0]}%;top:${POINTS[i][1] * 4 / 3}%">`).join('')}<span class="sparkle sparkle-one" aria-hidden="true"></span><span class="sparkle sparkle-two" aria-hidden="true"></span><span class="sparkle sparkle-three" aria-hidden="true"></span></div><button class="wear" data-target="wear" aria-label="Wear it with the fox"><span>Wear it!</span></button><div class="actions complete-actions">${action('again', 'Make another', 'shuffle')}${action('choose', 'Choose pattern', 'back')}</div></div></section>`);
