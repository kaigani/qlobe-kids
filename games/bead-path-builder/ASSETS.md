...
Bead Path Builder adds no image files, recorded clips, downloaded assets, generated art, or network assets. The current beta uses emoji beads and swatch cards rendered by the build-assemble engine.
...
| Emoji placeholder beads (`emoji:🔴`, `emoji:🟡`, `emoji:🔵`, `emoji:🟢`, `emoji:🟣`, `emoji:❤️`, `emoji:⭐`, `emoji:🔷`, `emoji:🔶`, `emoji:💚`, `emoji:💛`, `emoji:🌙`) | Rendered by local system emoji through `shared/js/engines/art.js` | Platform/browser emoji font vendors | Varies by device | No in-game attribution required for placeholder use | Used as temporary art refs only |
| Swatch placeholder beads | N/A - CSS color chips rendered locally by `shared/js/engines/art.js` | N/A | N/A | N/A | Hex colors supplied in config |
