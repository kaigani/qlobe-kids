...
| Runtime asset | Deterministic derivation | Provenance / QA |
|---|---|---|
| `assets/textures/wood-surface.webp` | GPT Image 2 `tray-empty.png`, clean top-wood crop `(500,0,756,180)` resized to 256x256; original/mirrored/flipped/mirror+flipped 2x2 quilt; 512x512 WebP q88/method 6 | Source crop and dimensions recorded in `assets/source/gpt-image-2/processing.json`; regenerated by `tools/process-assets.py` |
| `assets/textures/clay-surface.webp` | GPT Image 2 reference edit `clay-surface-gpt-image-2.png`, center crop `(371,371,883,883)`; deterministic low-frequency-light removal and flat `#79b52d` albedo blend preserve only irregular pore/fingerprint micro-detail; 2x2 mirror quilt; 1024x1024 WebP q88/method 6 | Full GPT Image 2 source, reference, prompt, crop, and finalization recipe recorded in `assets/source/gpt-image-2/processing.json`; regenerated by `tools/process-assets.py` |

These are authoring-time raster surfaces derived from approved GPT Image 2 sources; no runtime generation or remote request is involved.
