...
if (!configResponse.ok) throw new Error(`Freeze Focus Dance config failed: ${configResponse.status}`);
if (!linesResponse.ok) throw new Error(`Freeze Focus Dance lines failed: ${linesResponse.status}`);
