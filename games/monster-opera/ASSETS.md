...
| `assets/monsters/monster-01…12/dance.mp4` | 12 matching `new-monsters/monster-NN/dance.mp4` files | Finalized to 4.000s; 480² H.264 yuv420p, 20fps, faststart, video-only. These drive Composer idle videos and the pooled Listen idle renderer; every visible output layer uses CSS Screen blend mode. | User-supplied QLOBE project assets / CC BY 4.0 |
...
Monster Opera deliberately carries 12 idle dances and 36 short audiovisual performances because the supplied moving cast and the child's chosen performances are the game mechanic, not decorative video. Delivery videos are resized, duration-bounded, and video-only. Composer uses one idle video per cast member; Listen shares at most one dance decoder per unique monster and paints it into the repeated Screen-blended event layers under a 320-paints-per-second ceiling. Performance MP4s load near the viewport and on demand, while compact audio is decoded in the background after the start gesture. Source masters remain separated from runtime files.
