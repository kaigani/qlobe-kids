...
class SortIntoBinsGame {
  ...
  buildShell() {
    this.mountEl.innerHTML = `
      <section class="qk-sort qk-sort-splash qk-eng-root qk-eng-surface qk-eng-page" aria-label="${escapeAttr(this.config.title)}"></section>
      <section class="qk-sort qk-sort-play qk-eng-root qk-eng-surface qk-eng-play" hidden></section>
      <section class="qk-sort qk-sort-end qk-eng-root qk-eng-surface qk-eng-page" hidden></section>
    `;
    this.screens = createScreens({
      root: this.mountEl,
      screens: {
        splash: this.mountEl.querySelector('.qk-sort-splash'),
        play: this.mountEl.querySelector('.qk-sort-play'),
        end: this.mountEl.querySelector('.qk-sort-end'),
      },
      initial: 'splash',
  ...
  renderSplash() {
    ...
    splash.innerHTML = `
      <a class="qk-sort-home qk-sort-img-btn qk-eng-img-btn qk-eng-ico-home qk-eng-corner-tl" href="../../" aria-label="${escapeAttr(this.config.copy.home)}"></a>
      <div class="qk-sort-splash-center qk-eng-center">
        <div class="qk-sort-splash-art qk-eng-card qk-eng-card-glyph" aria-hidden="true">${escapeHtml(splashGlyph(this.config.splashArt))}</div>
        <h1 class="qk-eng-title">${escapeHtml(this.config.title)}</h1>
        <div class="qk-sort-mode-list qk-eng-mode-list"></div>
      </div>
    `;
    this.applyThemeBackdrop(splash);

    const picker = renderModeCards({
      host: splash.querySelector('.qk-sort-mode-list'),
      ...
      skin: false,
      cardClass: 'qk-sort-mode qk-eng-mode',
    ...
    const homeLink = splash.querySelector('a.qk-sort-home');
  ...
  renderPlayShell() {
    const dots = Array.from({ length: this.roundsTotal }, () => (
      '<span class="qk-sort-dot qk-eng-dot" aria-hidden="true"></span>'
    )).join('');
    ...
    play.innerHTML = `
      <header class="qk-sort-hud qk-eng-hud">
        <button class="qk-sort-back qk-sort-img-btn qk-eng-img-btn qk-eng-ico-back qk-eng-corner-tl" type="button" aria-label="Back to the game menu"></button>
        <div class="qk-sort-progress qk-eng-pill-wrap" aria-hidden="true">${dots}</div>
      </header>
      <main class="qk-sort-stage qk-eng-stage">
        <div class="qk-sort-canvas qk-eng-canvas" aria-label="${escapeAttr(this.mode.prompt || this.mode.title)}"></div>
      </main>
      <button class="qk-sort-img-btn qk-sort-sound qk-eng-img-btn qk-eng-ico-sound qk-eng-corner-bl" type="button" aria-label="${escapeAttr(this.config.copy.replay)}"></button>
    `;
    ...
    const sound = play.querySelector('.qk-sort-sound');
  ...
  async createPlayStage() {
    const host = this.screens.el('play').querySelector('.qk-sort-canvas');
  ...
  renderEnd(end) {
      ...
      <button class="qk-sort-back qk-sort-img-btn qk-eng-img-btn qk-eng-ico-back qk-eng-corner-tl" type="button" aria-label="Back to the game menu"></button>
      <div class="qk-sort-end-center qk-eng-center">
        <div class="qk-sort-end-art qk-eng-card qk-eng-card-glyph" aria-hidden="true">${escapeHtml(splashGlyph(this.config.endArt || this.config.splashArt))}</div>
        <h1 class="qk-eng-title">${escapeHtml(this.config.voice.cheer)}</h1>
        <button class="qk-sort-again qk-eng-mode" type="button">
          <span class="qk-sort-play-icon qk-eng-play-icon" aria-hidden="true"></span>
          <span>${escapeHtml(this.config.copy.playAgain)}</span>
        </button>
      </div>
    `;
    // `.qk-sort-back` is handled by the delegated onTap wired in the constructor
    // (it already covers every screen), so only "again" is wired here.
    ...
    wireEndScreen({
      ...
      again: end.querySelector('.qk-sort-again'),
      feedback: (e) => { e.preventDefault(); this.unlockAudio(); this.playSfx('tick'); },
      onAgain: () => (this.mode ? this.startMode(this.mode.id) : this.renderSplash()),
    });
    this.createDomBurst(end.querySelector('.qk-sort-end-art'), 34, end);
  }
  ...
  updateDots() {
    this.screens.el('play').querySelectorAll('.qk-sort-dot').forEach((dot, index) => {
...
function installStyle() {
  ...
  styleInstalled = true;
  installEngineStyles('qk-sort-style', `
    /* sort-into-bins's own skin. Everything the other engines also had —
       ...
       palette or a control only this engine has (the wrapping progress pill,
       the drag/bin/tray art, the confetti burst).
    ...
    .qk-sort-play-icon { margin-right: 10px; }
    ...
    .qk-sort-burst span {
      ...
      animation: qk-sort-burst .82s ease-out forwards;
