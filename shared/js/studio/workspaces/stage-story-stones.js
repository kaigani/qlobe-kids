...
export async function mount(host,{toast,setNav,setParam}){
  ...
  ids=normalize(ids);
  // Transport (Play/Stop + badge) overlays the canvas; the stone selects + Save/Export
  // live in a .sidebar-session panel OUTSIDE [data-inspector] so draw()'s rebuilds keep them.
  host.innerHTML=`<div class="workspace" data-workspace="stage"><div class="workspace-canvas" data-stage><div class="canvas-transport"><button data-play>Play exact story</button><button data-stop class="warn">Stop</button><span class="canvas-badge">Story Stones · 220 unordered complete stories</span></div></div><aside class="workspace-inspector"><div class="panel-section"><div class="sidebar-session"><label>Story stone<select data-slot="0"></select></label><label>Story stone<select data-slot="1"></select></label><label>Story stone<select data-slot="2"></select></label><button data-save class="save">Save Story Pack</button><button data-export class="ghost">Export JSON</button></div></div><div data-inspector></div></aside></div>`;
  const stageHost=host.querySelector('[data-stage]'),inspector=host.querySelector('[data-inspector]');
  // Breadcrumbs: SCENES ▸ <story title>, refreshed on every draw (selection change).
  const syncNav=()=>setNav({crumbs:[{label:'Scenes'},{label:selectedRecipe()?.title||'Story'}]});
