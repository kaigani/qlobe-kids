...
// workspace; the "Media (unassigned)" facet renders read-only cards — provenance
// only — through lib/generate-core.js's media controller, with an "Open in
// Generate" deep link on every card.

import { serverStatus } from '../api.js';
import { loadStudioObjects } from '../projects.js';
import { RIGGED_CHARACTERS } from './rig-data.js';
import { createMediaController, mediaToObject, escapeHtml, closeOverlay } from './lib/generate-core.js';
...
export async function mount(host, { params, toast, openWorkspace, setNav, setParam }) {
  ...
  // ---- media (WP-5c, read-only since Phase 6) ----
  // The controller owns the media list, the provenance panel state and every
  // /api/studio/media* call. readOnly:true renders cards without the mutation row;
  // the only extra affordance is the deep link into the Generate workspace's Review
  // tab, which is where accept / reject / assign / regenerate now live.
  const mediaController = createMediaController({
    ...
    extraActions: (item) => `<button type="button" class="ghost" data-media-action="open-in-generate" data-media-id="${escapeHtml(item.id)}">Open in Generate</button>`,
  });
      ...
      <aside class="workspace-inspector">
        <div class="panel-section">
          <div class="sidebar-filters">
            ...
            <button type="button" class="ghost wide" data-action="open-generate">Open Generate →</button>
          </div>
          <p class="hint">The <strong>project</strong> filter here narrows the cards below; the project chip in the breadcrumb bar scopes the whole studio.</p>
        </div>
        <div class="panel-section">
          <h2>Library</h2>
  ...
  function cardHtml(object) {
    if (object.type === 'media') return mediaController.mediaCardHtml(object);
  ...
  async function openObject(id) {
    ...
    if (!object) {
      // Media objects are read-only here — clicking one (or driving it through the
      // debug hook) opens its Provenance panel instead of navigating away. The
      // "Open in Generate" button on the card is the route to the editing loop.
      if (mediaController.has(id)) { mediaController.openPanel(id, 'provenance'); render(); }
      return;
    }
  ...
  // ============================================================================
  // Media objects — read-only browse (Phase 6).
  // The card markup, the provenance panel and every /api/studio/media* call live in
  // lib/generate-core.js; the Library instantiates that controller in read-only mode
  // and only routes the "Open in Generate" click. Nothing here mutates media.
  // ============================================================================

  // Deep link into the Generate workspace's Review tab. openWorkspace(id, {update})
  // takes shell options, not workspace state, so the section travels as a URL param
  // (studio.js §9.1 hygiene: the workspace validates ?section on mount).
  async function openInGenerate() {
    setParam('section', 'review');
    await openWorkspace('generate');
  }

  // ---- media event wiring — delegated on host, survives grid re-renders ----
  // `host` is the shell's persistent #native-workspace node (studio.js only clears
  // its innerHTML between mounts), so this listener MUST come off again in cleanup
  // or every revisit stacks another handler on the same node.
  let onHostClick = null;
  function wireMedia() {
    onHostClick = (event) => {
      if (event.target.closest('[data-action="open-generate"]')) { openInGenerate().catch(fail); return; }
      const mediaButton = event.target.closest('[data-media-action]');
      if (!mediaButton) return;
      event.stopPropagation();
      if (mediaButton.dataset.mediaAction === 'open-in-generate') { openInGenerate().catch(fail); return; }
  ...
  try {
    ...
    serverHint.textContent = serverAvailable
      ? 'Authoring server connected — use "check" on a card to validate it. Generated media is read-only here; open it in Generate to review, accept or assign it.'
      : 'Static preview — usage counts come from the on-disk usage index; validation and media browsing are unavailable.';
    // The Generate link is pure navigation, so it stays live in a static preview —
    // that workspace browses the template registry from the committed file and
    // disables its own server-dependent actions.
    wireFacets();
    wireMedia();
    render();
  } catch (error) {
  ...
  // Debug surface for browser automation (WP-2c gate, extended WP-5c for media).
  // Read-only since Phase 6: generate / regenerate / assign / accept / reject moved
  // to the Generate workspace's own debug object.
  ...
  return () => {
    destroyed = true;
    // The preview overlay a media card opens lives on document.body, outside
    // `host` — dropping the workspace must take it down with it.
    closeOverlay();
